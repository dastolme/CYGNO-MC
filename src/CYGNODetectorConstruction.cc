...
G4VPhysicalVolume* CYGNODetectorConstruction::Construct()
{
    ...
    else if (CYGNOShielding == "CYGNO_04") 
    {
      ...
      int numLayers = numLayers_;

      // Define the total thickness of all layers
      G4double totalThickness = numLayers * layerThickness;

      // Define the internal dimensions of the shielding
      G4double internal_x = internal_x_;
      G4double internal_y = internal_y_;
      G4double internal_z = internal_z_;
    ...
    ifstream infile(namestl);
    if (infile.good())
        mesh_water_shield = new CADMesh(namestl);
    infile.close();

    // pe_shield
    snprintf(namestl, sizeof(namestl), "%s/pe_shield.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    // camera_shield
    snprintf(namestl, sizeof(namestl), "%s/camera_shield.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    // lenses_ext
    snprintf(namestl, sizeof(namestl), "%s/lenses_ext.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    // lenses
    snprintf(namestl, sizeof(namestl), "%s/lenses.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    // pmts
    snprintf(namestl, sizeof(namestl), "%s/pmts.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    // acrylic_box
    snprintf(namestl, sizeof(namestl), "%s/acrylic_box.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    // cathode_frame
    snprintf(namestl, sizeof(namestl), "%s/cathode_frame.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    // fc_sheet
    snprintf(namestl, sizeof(namestl), "%s/fc_sheet.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    // internal_structure
    snprintf(namestl, sizeof(namestl), "%s/internal_structure.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    // gems
    snprintf(namestl, sizeof(namestl), "%s/gems.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    // gem_frame
    snprintf(namestl, sizeof(namestl), "%s/gem_frame.stl", CYGNOGeomPath.c_str());
    G4cout << namestl << G4endl;
    infile.open(namestl);
    if (infile.good())
    ...
    {   
      ...
      for (int i = 0; i < numLayers_; ++i) 
      {
        ...
        // Get the logical volume of the layer you want to move
        G4String layerName = "CuLayer_" + std::to_string(i);
        G4LogicalVolume* layerBox_log = layerLogical_[i]; // Retrieve the logical volume of the layer

        // Move the placement of the hollow box
        if (layerBox_log) 
        {
          G4VPhysicalVolume* layerPhysical = new G4PVPlacement(G4Transform3D(), layerBox_log, layerName, AirBox_log, false, 0, true);
          layerPhysical_.push_back(layerPhysical);

        } else {
            // Handle the case where physical volume is nullptr
            G4cerr << "Error: Existing physical volume of layer " << layerName << " is nullptr!" << G4endl;
            // You may choose to handle this error condition based on your application's requirements
        }
      }
...
void CYGNODetectorConstruction::SaveMassAndDensity()
{
  ...
  if (CYGNOShielding=="FullShield"){
    ...
    // Iterate over each logical layer and add its properties
    for (size_t i = 0; i < layerLogical_.size(); ++i) {
